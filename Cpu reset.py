...
def round_robin(processes, quantum):
    ...
    for process in processes:
        ...
        last_execution_time[process['processId']] = process['arrivalTime']  # Track when the process was last executed

    while len(completed_processes) < len(processes):
        # Add all processes that have arrived to the queue and haven't finished
        for process in processes:
            if process['arrivalTime'] <= current_time and process['processId'] not in completed_processes and process not in queue:
        ...
        process = queue.pop(0)  # Get the next process to execute
        
        # Calculate the actual execution time (either the quantum or the remaining burst time)
        exec_time = min(quantum, remaining_burst_time[process['processId']])
        
        # Update Gantt chart and current time
        gantt.append(process['processId'])
        current_time += exec_time
        remaining_burst_time[process['processId']] -= exec_time
        
        # Update waiting time for this process
        waiting_time[process['processId']] += current_time - last_execution_time[process['processId']] - exec_time
        last_execution_time[process['processId']] = current_time

        # If process finished, update turnaround time and mark it as completed
        if remaining_burst_time[process['processId']] == 0:
            turnaround_time[process['processId']] = current_time - process['arrivalTime']
            completed_processes.add(process['processId'])
        else:
            queue.append(process)  # Re-add the process to the queue if not completed
